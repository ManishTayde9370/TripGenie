...
class AmadeusService {
    ...
    private val clientId = BuildConfig.AMADEUS_CLIENT_ID
    private val clientSecret = BuildConfig.AMADEUS_CLIENT_SECRET
    ...
    /**
     * New: Search for locations (City/Airport) to get codes dynamically
     */
    fun searchLocations(token: String, keyword: String): String? {
        val url = "$baseUrl/v1/reference-data/locations?subType=CITY,AIRPORT&keyword=$keyword"
        val request = Request.Builder()
            .url(url)
            .addHeader("Authorization", "Bearer $token")
            .build()
