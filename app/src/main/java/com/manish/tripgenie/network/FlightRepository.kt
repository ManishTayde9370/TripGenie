...
class FlightRepository(private val amadeusService: AmadeusService) {
    ...
    /**
     * New: Get IATA code dynamically from city/airport name
     */
    suspend fun getIataCode(keyword: String): String? = withContext(Dispatchers.IO) {
        val token = getValidToken() ?: return@withContext null
        val response = amadeusService.searchLocations(token, keyword) ?: return@withContext null
    ...
    suspend fun searchFlights(
        ...
        try {
            // CPU intensive parsing on Default thread
